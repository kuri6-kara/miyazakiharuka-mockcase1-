...
```
 ...
 ・ItemRequest のバリデーションルールで、「0 円以上」とあったが、クライアントと検討した結果「100 円以上」へ変更しています。
　
　・ProfileRequest　のバリデーションルールで、プロフィール画像のルールに「2MBまでの画像ファイルはアップロード可」を、住所のルールに「255文字以内」を追加し、フォームに建物名を追加、ルールは「255文字以内」としています。
　
　・マイページの「出品した商品」「購入した商品」タブ選択時、それぞれ登録商品なければコメントを表示しています。

　・承認済ユーザーがマイリストタブを表示した場合、「いいね」した商品がなければ、コメントをヘッダーの下に表示しています。
